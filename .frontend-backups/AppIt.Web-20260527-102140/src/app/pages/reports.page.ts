...
export class ReportsPageComponent {
  ...
  readonly datasets: DatasetPreset[] = [
    ...
    },
    {
      label: 'Vouchers',
      path: '/api/vouchers',
      reportKey: 'voucher-summary',
      reportTitle: 'Voucher Summary Report'
    }
  ];

  private readonly maxRowsOnSinglePage = 100;
  ...
  exportPdf(): void {
    ...
    if (
      rows.length > this.maxRowsOnSinglePage &&
      !window.confirm(
        `This PDF contains ${rows.length} rows. It may span multiple pages and dense columns can be truncated. Continue export?`
      )
    ) {
      return;
    }
